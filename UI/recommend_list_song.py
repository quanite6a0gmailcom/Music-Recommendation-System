...
class recommend_list_song:
    def __init__(self,db_songs,his_db,clus_db,all_vector_audio,all_vector_lyrics,all_vector_cmt):
    ...
    def recommend_list(self):
        results = []
        top_cluster_id = self.get_top_cluster(self.his)
        songs = list(self.his.find({"cluster_id": top_cluster_id}))
        for song in songs:
            result = self.db_songs.find_one({"_id": ObjectId(song['song_id'])})
            results.append(result)
            result = self.recommend_hybrid(song['song_id'])
